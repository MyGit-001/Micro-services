...
routing rules are simple: match a path prefix, forward to a service via load-balancing (lb://ServiceName tells the gateway "resolve this via Eureka/discovery, then load-balance across instances").
...
.filters(...) lets you attach custom GatewayFilters (like an auth check or request logging) to specific routes only, instead of applying a filter globally.
...
the moment you need something like — "log the request body only on POST to /users," or "run an auth filter on some routes but not others," or "route based on a header value" — YAML predicates alone can't do it, and you'd write a RouteLocator bean like the one you found, mixing .and() conditions and .filters().
